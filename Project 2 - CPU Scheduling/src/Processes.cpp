...
void simulateScheduling(std::vector<Process> (*fun)(std::queue<Process>), int procs, int seed){
    ...
    for(const Process& proc : completedJobs){
        ...
        int wait = turnaround - proc.runtime;
...
//Function to run process N times and calculate averages
void simulateHPF(std::vector<Process> (*fun)(std::queue<Process>, bool), bool age, int procs, int seed){
    std::vector<Process> completedJobs;
    std::vector<Process> tempJobs;
    int totalRunTime = 0;
    int totalProcsCompleted = 0;
